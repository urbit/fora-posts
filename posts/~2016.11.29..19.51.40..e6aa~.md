---
type: post
date: ~2016.11.29..19.51.40..e6aa
title: The one true boot sequence (with comments)
...
For a long time it greatly distressed me that Urbit did not actually boot itself with a clean and perfect lifecycle function as described in the whitepaper.  However, there was nothing that actually depended on this.  So the task of cleaning it up kept getting pushed out.

But not indefinitely.  Here (from `cgyarvin/plastic-boot`) is a true, full, and correct boot sequence (with some extraneous printfs).  Unlike the version in the whitepaper, it actually works.

To be exact, this is a generator that builds the first N events in a legitimate Urbit event sequence.  The lifecycle formula that runs this is of course `[2 [0 3] [0 2]`:
