...
<ol>
  ...
  <li>For each seed, to derive public/private keys and addresses:
    <ul>
      ...
      <li>To derive an Ethereum address from the private key, use secp256k1 to get the uncompressed public key from the private key, then hash its last 64 bytes with keccak-256 and take the last 20 bytes from the result. If outputting addresses as strings rather than hex values, the strings should be checksummed as per [EIP-55 (https://github.com/ethereum/EIPs/blob/master/EIPS/eip-55.md).</li>
    </ul>
  </li>
